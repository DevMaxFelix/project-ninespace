...
const criaTiros = (posicaoLeftTiro, posicaoTopTiro) => {
    ...
    tiro.style.backgroundColor = "#ff9900";
...
const naveInimigas = () => {
    ...
    inimigo.setAttribute("data-vida", 3);
...
const colisao = () => {
    ...
    todasNavesInimigas.forEach((naveInimiga) => {
        todosTiros.forEach((tiro) => {
            ...
            let vidaAtualNaveInimiga = parseInt(
                naveInimiga.getAttribute("data-vida")
            );
            if (
                ...
                if (vidaAtualNaveInimiga === 0) {
                    ...
                    naveInimiga.remove();
                    naveInimigaDestruida(
                        posicaoNaveInimigaLeft,
                        posicaoNaveInimigaTop
                    );
                } else {
...
const naveInimigaDestruida = (
    posicaoLeftNaveInimiga,
    posicaoTopNaveInimiga
) => {
    ...
    audioExplosoes();
    setTimeout(() => {
        cenario.removeChild(naveInimigaDestruida);
    }, 1000);
};
...
const explosaoNaveInimigaDestruida = (posicaoLeftNaveInimiga) => {
    ...
    audioExplosoes();
    setTimeout(() => {
        cenario.removeChild(explosaoNaveInimiga);
    }, 1000);
};
